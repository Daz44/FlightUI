...
public class FLUIGeneric {
	...
	public void init() throws Exception {
		...
		try {
			...
			if (e.getMessage().equals(
				...
				throw new InvalidJVMConfigurationException(e.getMessage(),
						"Please start FlightUI with a 32BIT JVM!");
			} else {
				if (e.getCause().toString()
						.contains("dll in java.library.path")) {
					throw new InvalidDLLConfigurationException(e.getCause()
							.toString(),
							"Please make sure fsuipc.dll is placed within your working directory or %home%");
				} else {
					throw new FLUIException(e.getCause().toString(),
							e.getMessage());
				}
			}
		}
		...
		if (FSUIPC_CONNECTION == 0) {
			throw new FlightSimConnectionException("Flight Sim Not Found",
	 ...
	 * 
	 * @param isUndecorated
	 *            If the frames should be decorated or not
	 *            (JFrame.setUndecorated())
	 * @param isResizable
	 *            If the frames should be resizable or not
	 *            (JFrame.setResiazble())
	 * @throws Exception
	 */
	public void configFrame(boolean isUndecorated, boolean isResizable,
		...
		try {
			...
			Frame_VirtualHorizon.setIconImage(ImageIO.read(new File(
					"resources/FLUI.png")));
			...
			Frame_Autopilot.setIconImage(ImageIO.read(new File(
					"resources/FLUI.png")));
			...
			Frame_LightPanel1.setIconImage(ImageIO.read(new File(
					"resources/FLUI.png")));
			...
			Frame_LightPanel2.setIconImage(ImageIO.read(new File(
					"resources/FLUI.png")));
			...
			Frame_RadioPanel.setIconImage(ImageIO.read(new File(
					"resources/FLUI.png")));
			...
			Frame_ENGDisp_Gauge.setIconImage(ImageIO.read(new File(
					"resources/FLUI.png")));
			...
			Frame_CautionPanel
					.setTitle("FlightUI - CAUTION/WARNING PANEL (Airliner)");
			...
			Frame_CautionPanel.setIconImage(ImageIO.read(new File(
					"resources/FLUI.png")));
